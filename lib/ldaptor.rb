...
module LDAP
  ...
  def self.escape(term, escape_asterisks = true)
    term = term.utc.strftime("%Y%m%d%H%M%S.0Z") if term.respond_to?(:utc)
    term.to_s.gsub(/[()#{:* if escape_asterisks}\\\0-\37]/) {|l| "\\" + l[0].to_s(16) }
  end
  ...
  module Filter
    ...
    class Hash < Abstract
      ...
      attr_reader   :hash
      # If the hash includes <tt>:* => true</tt>, asterisks will be allowed to
      # pass through.  Otherwise, they are escaped.
      def initialize(hash)
        @hash = hash.dup
      ...
      def process
        ...
        string = @hash.stringify_keys.sort.map do |(k,v)|
          k = k.to_s.dup
          inverse = !!k.sub!(/!$/,'')

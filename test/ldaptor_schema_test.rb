...
class LdaptorSyntaxesTest < Test::Unit::TestCase
  ...
  def test_name_form
    name_form = Ldaptor::Schema::NameForm.new(NAME_FORM)
    ...
    assert name_form.inspect.include?("#<Ldaptor::Schema::NameForm 1.2.3 {")
  end

  def test_object_class
    assert_equal "AUXILIARY", Ldaptor::Schema::ObjectClass.new("(1.2 AUXILIARY)").kind
  end

  def test_attribute_type
    attribute_type = Ldaptor::Schema::AttributeType.new(ATTRIBUTE_TYPE)
    assert_equal 256, attribute_type.syntax_len
    assert_not_nil attribute_type.syntax
  end

  def test_parse_error
    assert_parse_error { Ldaptor::Schema::NameForm.new("x") }
    assert_parse_error { Ldaptor::Schema::NameForm.new("(1.2.3 NAME (foo | bar))") }
    assert_parse_error { Ldaptor::Schema::NameForm.new("(1.2.3 &)") }
  end
